...
import org.scoringengine.pfa.lib1.math.Round
...
package object core {
  ...
  object Comparison extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalComparison")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.Comparison(%s))", javaSchema(x, false))
      case _ =>
  ...
  object Equal extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalEQ")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonOperator(%s, 1))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  object GreaterOrEqual extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalGE")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonOperator(%s, 2))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  object GreaterThan extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalGT")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonOperator(%s, 3))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  }
  ...
  object NotEqual extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalNE")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonOperator(%s, -1))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
  ...
  object LessThan extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalLT")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonOperator(%s, -2))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
  ...
  object LessOrEqual extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.params.head match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble => 
        JavaCode("org.scoringengine.pfa.data.NumericalLE")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonOperator(%s, -3))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
  ...
  object Max extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.ret match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble =>
        JavaCode("org.scoringengine.pfa.data.NumericalMax")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonMax(%s))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
  ...
  object Min extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode = fcnType.ret match {
      case _: AvroInt | _: AvroLong | _: AvroFloat | _: AvroDouble =>
        JavaCode("org.scoringengine.pfa.data.NumericalMin")
      case x: AvroType =>
        JavaCode("(new org.scoringengine.pfa.data.ComparisonMin(%s))", javaSchema(x, false))
      case _ =>
        throw new Exception  // should never be called
    }
    override def javaCode(args: Seq[JavaCode], argContext: Seq[AstContext], paramTypes: Seq[Type], retType: AvroType): JavaCode = retType match {
