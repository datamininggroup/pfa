...
package shared {
  ...
  class SharedMapInMemory(initialCapacity: Int = 16, loadFactor: Float = 0.75f, concurrencyLevel: Int = 16) extends SharedMap {
    ...
    override def put[X](name: String, path: Array[PathIndex], value: X, initializing: Boolean = false): Either[Exception, Unit] = try {
      if (!initializing)
        blockUntilInitialized()
    ...
    override def remove(name: String, path: Array[PathIndex]): Either[Exception, Unit] = try {
      blockUntilInitialized()
      hashMap.remove(name)
      Right(())
    }
