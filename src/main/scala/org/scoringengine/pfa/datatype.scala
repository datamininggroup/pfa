...
import org.scoringengine.pfa.util.uniqueEnumName
...
package datatype {
  ...
  class AvroPlaceholder(original: String, forwardDeclarationParser: ForwardDeclarationParser) {
    ...
    private[pfa] def parser = forwardDeclarationParser
  }
  ...
  class ForwardDeclarationParser {
    ...
    def parse(jsons: Seq[String]): Map[String, AvroType] = {
      ...
      do {
        ...
        val goodTypes = preexistingTypes union findFullyParsedTypes(schemae.values.toList)
        types = parser.getTypes
        for (key <- types.keys.toSet if (!goodTypes(key)))
          types.remove(key)
