...
import pfa.lib1.array
import pfa.lib1.bytes
import pfa.lib1.core
import pfa.lib1.enum
import pfa.lib1.fixed
import pfa.lib1.impute
import pfa.lib1.map
import pfa.lib1.math
import pfa.lib1.record
import pfa.lib1.string
import pfa.lib1.stat.sample
...
class FunctionTable(object):
    ...
    def blank():
        functions = {}
        functions.update(pfa.lib1.array.provides)
        functions.update(pfa.lib1.bytes.provides)
        functions.update(pfa.lib1.core.provides)
        functions.update(pfa.lib1.enum.provides)
        functions.update(pfa.lib1.fixed.provides)
        functions.update(pfa.lib1.impute.provides)
        functions.update(pfa.lib1.map.provides)
        functions.update(pfa.lib1.math.provides)
        functions.update(pfa.lib1.record.provides)
        functions.update(pfa.lib1.string.provides)
        functions.update(pfa.lib1.stat.sample.provides)

        # TODO: functions.update(pfa.lib1.other.provides)...
...
class EngineConfig(Ast):
    ...
    def walk(self, task, symbolTable, functionTable):
        ...
        beginCalls = set(pfa.util.flatten([x[0].calls for x in beginContextResults]))
        ...
        endCalls = set(pfa.util.flatten([x[0].calls for x in endContextResults]))
...
class FcnDef(Argument):
    ...
    def walk(self, task, symbolTable, functionTable):
        ...
        context = self.Context(FcnType([self.params[k] for k in self.paramNames], self.ret), set(pfa.util.flatten([x[0].calls for x in results])), self.params, self.ret, scope.inThisScope, [x[1] for x in results])
        return context, task(context)
    ...
    @pfa.util.case
    class Context(AstContext):
        def __init__(self, fcnType, calls, params, ret, symbols, exprs): pass
...
class NewObject(Expression):
    ...
    def walk(self, task, symbolTable, functionTable):
        ...
        for name, expr in self.fields.items():
            exprContext, exprResult = expr.walk(task, symbolTable, functionTable)
            calls = calls.union(exprContext.calls)
...
class Log(Expression):
    ...
    def walk(self, task, symbolTable, functionTable):
        ...
        context = self.Context(AvroNull(), set(pfa.util.flatten([x[0].calls for x in results])).union(set([self.desc])), scope.inThisScope, results, self.namespace)
        return context, task(context)
