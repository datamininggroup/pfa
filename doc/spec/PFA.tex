...
\item[\PFAc zero:] Embedded JSON data whose type must match the {\PFAc output} type of the engine.  This is only used by the ``fold'' method to initialize the fold aggregation.
...
\begin{example}
This example implements a simple decision tree.  Input data are records with three fields: ``one'' (integer), ``two'' (double), and ``three'' (string).  The decision tree is stored in a cell named ``tree'' with type ``TreeNode''.  The tree has three binary splits (four leaves).  The scoring engine walks from the root to a leaf for each input datum, choosing a path based on values found in the input data, and returns the string it finds at the leaf.  (See the function definition of \hyperlink{model.tree.simpleWalk}{model.tree.simpleWalk} for a complete explanation.)
\begin{json}
