...
\item Embedded data are JSON objects, JSON arrays, numbers, integers, and strings that describe data structures.  The syntax and meaning of these objects are also defined by Avro, as the format used by the {\PFAc JSONEncoder} and {\PFAc JSONDecoder}.
\end{itemize}
...
Some special forms and library functions make use of some top-level fields.  For example, library functions that generate random numbers use the {\PFAc randseed} field for configuration.  These special forms and library functions cannot be considered implemented unless the corresponding top-level fields are also implemented.  These dependencies are explicitly defined in this specification.
...
The PFA standard is defined so that a PFA-compliant system can verify that the JSON types of a PFA document are correctly composed (syntax check), verify that the PFA invariants and Avro data types are correctly composed (semantic check), and impose additional constraints on the set of top-level fields, special forms, and library functions used.  A PFA-compliant system should perform the syntax and semantic checks, including all type inference and type checking, but it is not required.  The behavior of a PFA document that does not satisfy these invariants and type constraints is not defined and such a document is not valid.  The third set of checks, however, is completely optional and different systems may apply different constraints on the kinds of scoring engines they are willing to execute.  However, the reasons why a PFA document is rejected should be explained.
...
A PFA document is a serialized JSON object representing an executable scoring engine.  Only the following JSON object member names may appear at this JSON nesting level.  These are the top-level fields referred to in the \hyperlink{sec:conformance}{conformance section} of this specification.  Three fields, {\PFAc action}, {\PFAc input}, and {\PFAc output}, are required for every PFA document and are therefore required for any PFA implementation.  The rest are optional for PFA documents and not strictly required for PFA implementations.  As explained in the conformance section, not implementing some top-level fields can make some special forms and functions unimplementable.

\begin{center}
\begin{minipage}{0.9\linewidth}
\begin{description}
\item[\PFAc name:] A string used to identify the scoring engine.
\item[\PFAc method:] A string that may be ``map'', ``emit'', or ``fold'' (\hyperlink{sec:method}{see below}).  If absent, the default value is ``map''.
\item[\PFAc input:] An Avro schema representing the data type of data provided to the scoring engine (\hyperlink{sec:input-output}{see below}).
\item[\PFAc output:] An Avro schema representing the data type of data produced by the scoring engine (\hyperlink{sec:input-output}{see below}).
\item[\PFAc begin:] An \hyperlink{sec:expressions}{expression} or JSON array of \hyperlink{sec:expressions}{expressions} that are executed in the begin phase of the scoring engine's run (\hyperlink{sec:phases}{see below}).
\item[\PFAc action:] An \hyperlink{sec:expressions}{expression} or JSON array of \hyperlink{sec:expressions}{expressions} that are executed for each input datum in the active phase of the scoring engine's run (\hyperlink{sec:phases}{see below}).
\item[\PFAc end:] An \hyperlink{sec:expressions}{expression} or JSON array of \hyperlink{sec:expressions}{expressions} that are executed in the end phase of the scoring engine's run (\hyperlink{sec:phases}{see below}).
\item[\PFAc fcns:] A JSON object whose member values are \hyperlink{sec:fcndef}{function definitions}, defining routines that may be called by expressions in {\PFAc begin}, {\PFAc action}, {\PFAc end}, or other functions.
\item[\PFAc zero:] Embedded JSON data whose type must match the {\PFAc output} type of the engine.  This is only used by the ``fold'' method (\hyperlink{sec:method}{see below}).
\item[\PFAc cells:] hey
\item[\PFAc pools:] hey
\item[\PFAc randseed:] hey
\item[\PFAc doc:] hey
\item[\PFAc metadata:] hey
\item[\PFAc options:] hey
\end{description}
\end{minipage}
\end{center}
...
\hypertarget{sec:input-output}{}
\subsection{Input and output type specification}

\hypertarget{sec:phases}{}
...
\hypertarget{sec:fcndef}{}

\hypertarget{sec:fcnref}{}
