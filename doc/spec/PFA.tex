...
Symbol names must match the following pattern: {\PFAc [A-Za-z\_][A-Za-z0-9\_]*} (the first character must be a letter or underscore; subsequent characters, if they exist, may also be numbers).
...
New functions can be defined in a PFA document and can be called as though they were library functions.  



Function names must match the following pattern: {\PFAc [A-Za-z\_]([A-Za-z0-9\_]|\textbackslash.[A-Za-z][A-Za-z0-9\_]*)*} (consists of period-delimited words in which each word starts with a letter or underscore; subsequent characters, if they exist, may also be numbers; there must be at least one word).

(\hyperlink{hsec:generic-signatures}{see Sec.~\ref{sec:generic-signatures}})
