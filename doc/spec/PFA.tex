...
Special forms are similar to function calls in that they take arguments, perform a function, and return a value, but special forms can have more than one JSON object member and can apply custom interpretations to their arguments.  A function always has the same form: one member name and an expression or array of expressions.  Some library functions can apply custom rules to the evaluation of their arguments: the {\PFAc and} function only evaluates its second argument if the first does not evaluate to {\PFAc false} and the {\PFAc or} function only evaluates its second argument if the first does not evaluate to {\PFAc true}.
...
\end{json}
where {\PFAc SYMBOL} is a string (not an expression) that specifies a pre-existing symbol and {\PFAc INDEXES} is a JSON array of {\bf int} expressions, {\bf string} expressions, and string literals that specifies a path through the nested objects.  If nesting is only one level deep, the {\PFAc path} must have exactly one element.  It is a syntax error for the {\PFAc path} to have zero elements.

If the type at a particular nesting level is an {\bf array}, the path index must resolve to an {\bf int}, though it can perform arbitrary calculations to produce the {\bf int}.  If the type at a particular nesting level is a {\bf map}, the path index must resolve to a {\bf string}.  If the type at a particular nesting level is a {\bf record}, the path index must be a literal {\bf string} (not an arbitrary expression), and that string must be a field name of the record.  Since it is a literal string, a static analysis can verify that the field name exists and a PFA system should raise a type error if no such field exists.  The nesting level is increased for each array element of the path, and the return type of ``attr'' is the type of the deepest object referenced by the path.

As a convenience, ``attr'' forms may also be expressed as a period-delimited string resembling a symbol reference.  The first substring is taken to be {\PFAc SYMBOL} and the rest are taken to be elements of the {\PFAc path} array.  PFA systems should implement this short-cut.
...
\end{json}
then the example is valid and the return type is {\PFAc X}.
\end{example}
...
\end{json}
with the same meaning for {\PFAc SYMBOL} and {\PFAc INDEXES}.  If the attribute evaluates to {\PFAc X} at the end of its path, the {\PFAc VALUE-OR-FUNCTION} argument must either be an expression of type {\PFAc X} or a function that maps {\PFAc X} to {\PFAc X}.  If is an expression, then the form will return a structure like the original, but with the specified subelement changed to its value.  If {\PFAc VALUE-OR-FUNCTION} is a function, then it must map from {\PFAc X} to {\PFAc X}.  The function is evaluated, passing the old value of the subelement and updating the structure with its return value.

The path must contain at least item.  There is no equivalent of using the period-delimited string as a short-cut.  The ``attr-to'' form returns a whole new structure with one subelement changed; it has the same type as {\PFAc SYMBOL}.
