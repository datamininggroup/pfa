...
\end{json}
The {\PFAc STRING} is the error message, and if a {\PFAc code} is provided, it provides a safer way to identify specific errors than a human-readable message.
...
{\bf FIXME: the ``code'' will either be dropped or all library errors will be given codes as well.}
...
or
\begin{json}
{"try": EXPRESSION-OR-EXPRESSIONS, "filter": ARRAY-OF-STRINGS}
\end{json}
...
If a ``filter'' is provided, only the specified error messages would be caught--- the rest are passed through.

{\bf FIXME: if library errors are given codes, then ``filter'' should accept strings or integers and use both for filtering.}
...
\subsection{Random integer (rand.int)}
\libfcn{rand.int}

\subsection{Random integer (rand.long)}
\libfcn{rand.long}

\subsection{Random integer (rand.float)}
\libfcn{rand.float}

\subsection{Random integer (rand.double)}
\libfcn{rand.double}

\subsection{Random string (rand.string)}
\libfcn{rand.string}

\subsection{Random bytes (rand.bytes)}
\libfcn{rand.bytes}

\subsection{Type-4 UUID (rand.uuid)}
\libfcn{rand.uuid}

\subsection{Gaussian deviates (rand.gaussian)}
...
\libfcn{time.fromUTCToLocal}

\subsubsection{Convert to UTC (time.fromLocalToUTC)}
\libfcn{time.fromLocalToUTC}
